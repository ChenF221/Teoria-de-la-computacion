...
# Cambiamos la función para guardar en formato CSV
def guardarRutasEnArchivo(rutas, nombre_archivo):
    with open(nombre_archivo, 'w', newline='') as archivo:  # Agregamos newline=''
        writer = csv.writer(archivo)
        for ruta in rutas:
            writer.writerow(ruta[1:])  # Guardamos solo las rutas sin el estado inicial
...
def limpiar(archivo_entrada, archivo_salida, numero, max_length):
    ...
    if lines:  # Verificar si hay líneas en el archivo
        ...
        filtered_lines = []
        for line in lines:
            line_length = len(line.strip().split())  # Contamos las palabras
...
def main():
    nombre_archivo1 = 'Bloque_1\\Tablero\\rutas_jugador1.csv'  # Cambiamos a .csv
    nombre_archivo2 = 'Bloque_1\\Tablero\\rutas_jugador2.csv'  # Cambiamos a .csv
    nombre_archivo1_reconfigurado = 'Bloque_1\\Tablero\\rutas1_reconfiguradas.csv'  # Cambiamos a .csv
    nombre_archivo2_reconfigurado = 'Bloque_1\\Tablero\\rutas2_reconfiguradas.csv'  # Cambiamos a .csv
